...
public class SigningUpPanel extends JPanel {
	...
	private class ConnectAction implements ActionListener{	
		public void actionPerformed(ActionEvent e) {
			if(loginField.getText().length()>0 && passwordField.getPassword().length>0 && passwordField2.getPassword().length>0) {
				if(passwordField.getPassword().equals(passwordField2.getPassword())) {
					//on enregistre dans bdd et on passe a l'ecran de debut de jeu
	...
	public void initLayout () {
		...
		passwordLabel2.setBounds(275, 214, 335, 76);
		add(passwordLabel2);
